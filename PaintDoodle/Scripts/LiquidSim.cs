...
namespace PaintDoodle
{
    ...
    ///   命中点世界坐标 -> PaintableSurface.PaintAtWorld 落颜料（世界投影贴花，不需 uv）。
    /// 生命周期：
    ...
    public class LiquidSim : MonoBehaviour
    {
            ...
            while (i < count)
            {
                ...
                else
                {
                        ...
                        h.collider.GetComponent<PaintableSurface>() == ps)
                    {
                        ...
                        ps.PaintAtWorld(h.point, d.r * 2.3f, d.col, wetAmt);
                    }
                    ...
                    if (!onSurf || stopped)
                    {
                        ...
                        if (onSurf && ps != null)
                            ps.PaintAtWorld(h.point, d.r * 5.5f, d.col, 0.6f);
                        remove = true;
                    }
                }
        ...
        void LandOnSurface(ref Drop d, RaycastHit hit, bool splashEnabled, ref bool remove)
        {
            ...
            {
                // 高速撞击：主溅 + 派生小飞沫
                ps.PaintAtWorld(hit.point, d.r * 3.6f, d.col, 0.75f);
            ...
            {
                // 中速：啪地摊成一大滩（黏稠颜料不碎）
                ps.PaintAtWorld(hit.point, d.r * 3.0f, d.col, 0.55f);
            }
            ...
            else
            {
                ...
                if (impact > 0.4f || !big)
                    ps.PaintAtWorld(hit.point, d.r * 6.0f, d.col, 0.55f);
                remove = true;
            }
        }
    }
